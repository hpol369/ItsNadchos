 ...
 */
export async function checkMessageCredits(userId) {
 ...
 */
export async function incrementTodayMessages(userId) {
    const today = new Date().toISOString().split('T')[0];
 ...
 */
export async function deductMessageCredit(userId) {
    // Get current balance
    const { data: credits } = await supabase
        .from('nacho_credits')
 ...
 */
export async function addCredits(userId, amount, type, referenceId) {
 ...
 */
export async function unlockPhoto(userId, photoId) {
 ...
 */
export async function getUnlockedPhotos(userId) {
    const { data } = await supabase
        .from('nacho_unlocked_photos')
        .select('photo_id')
        .eq('user_id', userId);
 ...
 */
export async function isPhotoUnlocked(userId, photoId) {
    const { data } = await supabase
        .from('nacho_unlocked_photos')
        .select('id')
        .eq('user_id', userId)
