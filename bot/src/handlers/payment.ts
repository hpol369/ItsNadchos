...
export async function handleSuccessfulPayment(ctx: Context) {
  ...
  if (!payment || !telegramUser) return;
  ...
  try {
    ...
    // Determine new tier
    let newTier: 'tier1' | 'tier2' | 'vip' = 'tier1';
    if (packId === 'bundle') {
      newTier = 'vip';
    ...
    const randomMessage = PHOTO_DELIVERED_MESSAGES[
      Math.floor(Math.random() * PHOTO_DELIVERED_MESSAGES.length)
    ];
