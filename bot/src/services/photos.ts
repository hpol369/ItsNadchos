...
import type { PhotoPack, Photo } from '../db/client.js';

const PAYMENT_PROVIDER_TOKEN = process.env.TELEGRAM_PAYMENT_PROVIDER_TOKEN || '';

export async function showPhotoPacks(ctx: Context): Promise<void> {
  // Get available packs
  ...
  let ownedPacks: Set<string> = new Set();
...
export async function sendPackInvoice(
  ctx: Context,
  userId: string,
  packId: string
): Promise<void> {
  ...
  try {
    await ctx.replyWithInvoice(
      pack.name,
      pack.description || `Get ${pack.photo_count} exclusive photos`,
      packId, // payload
      'USD',
      [{ label: pack.name, amount: pack.price_cents }],
      {
...
export async function redeliverPhotos(ctx: Context, userId: string): Promise<void> {
